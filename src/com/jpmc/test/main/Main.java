...
public class Main {
	...
	private static void displayReportDailyTotal(Map<LocalDate, BigDecimal> dailyTotalMap, String title) {
		...
		dailyTotalMap.entrySet().forEach(entry -> {
			System.out.println(String.format("Date %s => total: %s", entry.getKey(), String.format("%10s", entry.getValue())));
		});
	}
	...
	private static void displayRankingIncomingTrades(List<Trade> trades, String title) {
		...
		trades.forEach(entry -> {
			System.out.println(String.format("%s -> total: %s", String.format("%-15s", i.getAndIncrement() + ". " + entry.getEntity()), String.format("%10s", entry.computeTotal())));
		});
	}
